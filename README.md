...
- フル指定: `8.3.12`
- ブランチ指定: `8.3`（`install` ではそのブランチの最新パッチに解決されます）
- `latest`（リモートで確認できる最新バージョン、`install` のみ）

`--ts` / `--nts` を省略した場合は `phpbrew config threading` で設定した既定値が使われます（初期値は `nts`）。同じバージョンでも TS 版・NTS 版は別バージョンとして共存インストールできます。

`phpbrew use` にも `8.3` のようなブランチ指定ができます。インストール済みバージョンの中から `8.3.*` に合致するもっとも新しいリビジョンが選ばれ、そのリビジョンに TS 版・NTS 版が両方インストールされている場合は `--ts`/`--nts` の指定、それも無ければ `phpbrew config threading` の設定値で選ばれます。該当するバージョンが一つもインストールされていない場合はエラーになります。`uninstall`/`protect`/`unprotect` はこれまで通りフル指定のみです。
...
# インストール済みの 8.3 系の中から最新リビジョンに切り替え
phpbrew use 8.3
